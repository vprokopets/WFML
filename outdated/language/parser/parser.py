...
class Parser():
    ...
    def preprocessing(self, tokens):
        ...
        strings = self.string_compose(tokens)
            ...
            for token in self.filter_by_depth(tokens, indent):
                if parent:
                    ...
                    for token_u in upper_indent_tokens:
                        if token in self.modulize_by_id(tokens, token_u) and token['tag'] == 'CLAFER':
                            token['parent'] = token_u['id']
                if child:
                    lower_indent_tokens = self.filter_by_depth(tokens, indent + 1)
                    print(lower_indent_tokens)
                    print(self.modulize_by_id(tokens, token))
                    childs = []
                    for token_l in lower_indent_tokens:
                        if token_l in self.modulize_by_id(tokens, token) and token_l['tag'] == 'CLAFER':
    ...
    def string_compose(self, tokens):
        strings = []
        string = []
        line = tokens[0]['line']
        for token in tokens:
    ...
    def clafer_decompose(self, string):
        clafer = Clafer()
        # Abstract
        if clafer[0]['token'] == 'abstract':
            print('This string has abstract token')
            clafer.abstract = True
